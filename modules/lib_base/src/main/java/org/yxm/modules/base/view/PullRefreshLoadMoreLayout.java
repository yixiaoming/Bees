...
public class PullRefreshLoadMoreLayout extends ViewGroup {
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    for (int i = 0; i < getChildCount(); i++) {
      ...
      if (child == mHeader) {
        ...
        child.layout(0, mContentHeight, child.getMeasuredWidth(),
    ...
    switch (ev.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (y > mLastY) {
          ...
          if (intercept && mState == STATE_RESET) {
            mState = STATE_TRY_REFRESH;
          }
        }
        // 向上刮动判断拦截加载更多
        else {
  ...
  private void moveLayoutToHeaderHeight() {
    ...
    ObjectAnimator.ofInt(this, "scrollY", getScrollY(), -mHeader.getHeight())
  ...
  private boolean shouldInterceptLoadmore(View child) {
    if (child instanceof RecyclerView) {
      return ((RecyclerView) child).computeVerticalScrollExtent() + ((RecyclerView) child)
